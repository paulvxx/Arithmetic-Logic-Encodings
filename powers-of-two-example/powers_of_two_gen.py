...
def print_powers_of_two_cycle(full_iters : int):
    ...
    for _ in range(full_iters):
        ...
        next_sequence = []
        for i in range(clen):
            next_sequence[i] = m1[current_sequence[i]]
        ...
        next_sequence_1 = []
        next_sequence_2 = []
        current_sequence_extended = copy.copy(current_sequence)
        for i in range(clen):
            next_sequence_1[i] = m2[current_sequence[i]]
            next_sequence_2[i] = m3[current_sequence[i]]
            current_sequence_extended.append(current_sequence[i])

        # order doesn't matter
        next_sequence = next_sequence_1.extend(next_sequence_2)
